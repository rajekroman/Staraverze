(() => {
  ...
  function resume(){if(mode!=="pause")return;mode=pausedMode;showOnly(mode==="dig"?screens.dig:null);audio.resumeMusic();setPlaying(mode==="playing");if(mode==="playing"){try{canvas.focus({preventScroll:true});}catch{canvas.focus();}}last=performance.now();}
